...
public class SwitchButton extends CompoundButton {
    ...
    public SwitchButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes
                (attrs,R.styleable.SwitchButton, defStyle, 0);// 从配置文件中获取相关配置
        mThumbDrawable = a.getDrawable(R.styleable.SwitchButton_my_thumb);// 获取配置的轨迹资源
        mTrackDrawable = a.getDrawable(R.styleable.SwitchButton_my_track);// 获取配置的开关资源
        mTextOn = a.getText(R.styleable.SwitchButton_my_textOn);// 获取配置中 选中情况下的显示文字on
        mTextOff = a.getText(R.styleable.SwitchButton_my_textOff);// 获取配置的 为选中情况下的显示文字off
        mThumbTextPadding = a.getDimensionPixelSize(
                R.styleable.SwitchButton_my_thumbTextPadding, 0);// 获取配置的按钮字体大小
        mSwitchMinWidth = a.getDimensionPixelSize(
                R.styleable.SwitchButton_my_switchMinWidth, 0);// 获取配置的最小宽度
        mSwitchPadding = a.getDimensionPixelSize(
                R.styleable.SwitchButton_my_switchPadding, 0);// 获取配置的padding属性

        int appearance = a.getResourceId(
                R.styleable.SwitchButton_my_switchTextAppearance, 0);// 获取配置文件中获取显示字体格式
        // 判断设定格式是否存在，存在的话就赋值
        if (appearance != 0) {
